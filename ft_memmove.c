...
- unsigned char *d y unsigned char *s son punteros a los datos de destino y origen, respectivamente, y se utilizan para acceder a los datos byte a byte.
- if (!dst && !src) return (NULL); es un chequeo de seguridad para asegurarse de que tanto src como dst no son NULL.
	* Si alguno de ellos es NULL, la función retorna NULL
-if (s < d && d < s + len) es un chequeo que se realiza para ver si la memoria de origen y destino se superpone.
	* Si se superpone y el origen está antes que el destino, entonces copia los datos de atrás hacia adelante para evitar la sobreescritura de los datos que aún no se han copiado.
- while (len--) *(d++) = *(s++); es el caso cuando la memoria de origen y destino no se superpone, o sí se superpone pero el destino está antes que el origen.
